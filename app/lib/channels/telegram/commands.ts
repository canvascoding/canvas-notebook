...
async function handleStopCommand(ctx: Context): Promise<void> {
  ...
  try {
    ...
    await ctx.reply(`Fehler beim Abbrechen: ${error instanceof Error ? error.message : 'Unbekannter Fehler'}`);
  }
}
...
async function handleCompactCommand(ctx: Context): Promise<void> {
  ...
  try {
    ...
    await ctx.reply(`Fehler beim Compacting: ${error instanceof Error ? error.message : 'Unbekannter Fehler'}`);
  }
}
...
async function handleStatusCommand(ctx: Context): Promise<void> {
  ...
  try {
    ...
    await ctx.reply(`Status: Fehler — ${error instanceof Error ? error.message : 'Unbekannt'}`);
  }
}
