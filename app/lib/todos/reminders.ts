...
export async function sendDueTodoReminders(now = new Date()): Promise<{ sent: string[]; failed: string[] }> {
  ...
  for (const todo of due) {
    ...
    try {
      ...
      const result = await db.update(todoItems).set({
        reminderSentAt: now,
        reminderError: null,
        updatedAt: now,
      }).where(and(eq(todoItems.id, todo.id), eq(todoItems.status, 'open'), isNull(todoItems.reminderSentAt))).returning({ id: todoItems.id });
      if (result.length > 0) sent.push(todo.id);
    } catch (error) {
