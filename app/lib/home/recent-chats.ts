...
export async function listHomeChats(userId: string, workspace: WorkspaceContext, query: string, limit: number): Promise<HomeChatPage> {
  ...
  const agentIds = [...(await listAgentAccessForUser(userId, workspace)).entries()]
  ...
  const piRows = await db.select({
    sessionId: piSessions.sessionId, title: piSessions.title, activityAt,
  ...
  if (workspace.workspaceType === 'personal' && agentIds.includes(DEFAULT_AGENT_ID) && await legacyAiTablesExist()) {
    ...
    chats.push(...legacy.map(row => ({ sessionId: row.sessionId, title: row.title, activityAt: row.lastMessage.getTime(), hasUnread: false })));
  }
