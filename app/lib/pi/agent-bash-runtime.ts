...
import { executeAgentSandboxedProcess } from '@/app/lib/pi/agent-shell-sandbox';
...
async function enforceAgentRuntimeTempQuotaAfterCommand(
  tempDir: string,
  ...
  try {
    ...
    try {
      if (executionContext) {
        const { preserveOfficeDocumentDraftsBeforeScratchCleanup } = await import('@/app/lib/pi/office-document-workflow');
        await preserveOfficeDocumentDraftsBeforeScratchCleanup(executionContext, tempDir);
      }
...
export async function executeAgentBashCommand(params: {
  ...
  try {
    if (sandboxMode === 'local-development') {
      const result = await executeAgentSandboxedProcess({ executable: '/bin/bash', args: ['-lc', params.command] }, {
    ...
    // Keep the read allowlist inside the native write/metadata confinement.
    // Kernel restrictions intersect, so neither layer can widen the other.
    const result = await executeAgentSandboxedProcess({ executable: launcherPath, args }, {
      context: params.executionContext,
      cwd: params.tempDir,
      env: params.env,
      signal: params.signal,
    ...
    try {
      if (params.tempDir) {
        await enforceAgentRuntimeTempQuotaAfterCommand(params.tempDir, params.executionContext);
      }
    } finally {
      releaseTempLease();
    }
