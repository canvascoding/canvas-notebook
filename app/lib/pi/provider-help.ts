...
export const PROVIDER_HELP: Record<string, ProviderHelpInfo> = {
  ...
  ollama: {
    ...
    setupSteps: [
      'Install Ollama locally or use Ollama Cloud',
      'Pull your desired model: ollama pull llama3.1',
      'Ensure Ollama server is running: ollama serve',
      'Verify status: curl http://localhost:11434/api/tags',
      'For remote/cloud: Set OLLAMA_HOST environment variable',
    ],
    envVars: [
      { 
        name: 'OLLAMA_HOST', 
      ...
      { 
        name: 'OLLAMA_API_KEY', 
        description: 'Optional API key for Ollama Cloud or authenticated endpoints', 
        scope: 'agents', 
        required: false 
      },
    ],
    ...
    notes: [
      ...
      'Default URL for local: http://127.0.0.1:11434 (native API, not /v1)',
      'Use OLLAMA_HOST to connect to remote Ollama instances',
      'For Ollama Cloud: Set OLLAMA_HOST and OLLAMA_API_KEY',
      'Models must be pulled before use with: ollama pull <model>',
      'Cloud models available: GLM 5.6, Kimi K2.5, Qwen 3.5 (via Ollama Cloud)',
      'Native Ollama API is preferred over OpenAI-compatible mode for better tool calling',
      'Requires sufficient local resources (RAM/VRAM) for local models',
      'Use "ollama ps" to check which models are currently loaded in memory',
    ],
