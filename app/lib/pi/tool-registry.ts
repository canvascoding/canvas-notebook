...
import { createDefaultManifest, ParameterDefinition } from '../skills/skill-manifest';
import { createSkillDirectory, createSkillReadme, validateSkillByName, skillExists } from '../skills/skill-loader';
...
export const piTools: AgentTool[] = [
  ...
  // Skill Creator Tools
  {
    ...
    description: 'Creates a new Canvas Notebook skill with manifest and documentation. Use when user wants to create a custom skill, add new functionality, or extend Canvas Notebook capabilities. The skill will be available immediately after creation.',
    parameters: Type.Object({
      name: Type.String({ description: 'Skill name (kebab-case, e.g., "text-to-speech")' }),
      title: Type.String({ description: 'Human-readable title (e.g., "Text to Speech")' }),
      description: Type.String({ description: 'Skill description with trigger phrases explaining when to use it' }),
      type: Type.String({ enum: ['cli', 'api'], description: 'Skill type: cli (local command) or api (API endpoint)' }),
      parameters: Type.String({ description: 'JSON object defining tool parameters (e.g., {"text": {"type": "string", "required": true}})' }),
      author: Type.Optional(Type.String({ description: 'Skill author (optional)' })),
    }),
    execute: async (toolCallId, params) => {
      const { name, title, description, type, parameters: paramsJson, author } = params as {
        name: string;
        title: string;
        description: string;
        type: 'cli' | 'api';
      ...
      try {
        ...
        if (!/^[a-z][a-z0-9-]*$/.test(name)) {
          return {
            content: [{ type: 'text', text: 'Error: Skill name must be kebab-case (lowercase letters, numbers, hyphens), starting with a letter' }],
            details: { error: 'Invalid name format' },
          };
        }
        ...
        if (await skillExists(name)) {
          return {
            content: [{ type: 'text', text: `Error: Skill "${name}" already exists` }],
            details: { error: 'Skill already exists' },
          };
        }
        
        // Parse parameters
        let parameters: Record<string, ParameterDefinition>;
        try {
          parameters = JSON.parse(paramsJson);
        } catch (e) {
          return {
            content: [{ type: 'text', text: 'Error: Invalid JSON in parameters field' }],
            details: { error: 'Invalid JSON' },
          };
        }
        
        // Create manifest
        const manifest = createDefaultManifest(name, title, description, type, author);
        manifest.tool.parameters = parameters;
        
        // Create skill directory and manifest
        const result = await createSkillDirectory(name, manifest);
        if (!result.success) {
          return {
        ...
        // Create README
        await createSkillReadme(name, manifest);
        
        return {
          content: [{ 
            type: 'text', 
            text: `✅ Skill "${name}" created successfully!\n\nLocation: /data/skills/${name}/\nManifest: /data/skills/${name}/manifest.json\nREADME: /data/skills/${name}/README.md\n\nThe skill is now available in the Skill Gallery and can be used immediately.` 
          }],
          details: { 
            name, 
            title, 
            type, 
            manifestPath: `/data/skills/${name}/manifest.json`,
            readmePath: `/data/skills/${name}/README.md`
          },
        };
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        return {
          content: [{ type: 'text', text: `Error creating skill: ${message}` }],
          details: { error: message },
        };
      }
    },
  ...
  {
    name: 'validate_skill',
    label: 'Validating skill',
    description: 'Validates a skill manifest to ensure it follows the correct schema and can be loaded. Use to check if a skill is properly configured before using it.',
    parameters: Type.Object({
      ...
      try {
        ...
        if (result.valid) {
          return {
            content: [{ 
              type: 'text', 
              text: `✅ Skill "${name}" is valid!\n\nTitle: ${result.manifest?.title}\nVersion: ${result.manifest?.version}\nType: ${result.manifest?.type}\nTool: ${result.manifest?.tool.name}\nParameters: ${Object.keys(result.manifest?.tool.parameters || {}).join(', ') || 'none'}` 
            }],
            details: { 
              valid: true, 
          ...
          return {
            content: [{ 
              type: 'text', 
              text: `❌ Skill "${name}" validation failed:\n\n${result.errors.join('\n')}` 
            }],
            details: { 
              valid: false, 
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        return {
          content: [{ type: 'text', text: `Error validating skill: ${message}` }],
          details: { error: message },
        };
      }
    },
