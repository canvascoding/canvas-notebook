...
function collectOutcomeFields(values: unknown[], maxChars: number): Record<string, unknown> {
  ...
  const visit = (entry: unknown, prefix: string, depth: number) => {
    ...
    for (const [key, nested] of entries) {
      ...
      if (/(?:^|_)(?:id|url|uri)$|^(?:isError|error|success|successful|status|code|auth_required|redirect_url|toolkit|toolkit_name|tool_name|profile_name|profile_source|connected|mutates)$/u.test(key)
        && (typeof nested === 'string' || typeof nested === 'boolean' || typeof nested === 'number')) {
        // Never turn a shortened URL into a seemingly usable original URL.
        const text = typeof nested === 'string' && !/(?:url|uri)$/u.test(key) ? clipToolText(nested, 300) : nested;
...
export async function prepareToolOutput(input: ToolOutputPreparationContext & {
  ...
  if (result.details && typeof result.details === 'object' && 'toolOutput' in result.details) {
  ...
  if (text.length <= limit && detailSize <= TOOL_OUTPUT_LARGE_RESULT_MAX_CHARACTERS) return markPreparedToolOutput(result);
