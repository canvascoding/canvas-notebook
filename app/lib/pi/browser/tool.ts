...
export function createBrowserGatewayTool(
  ...
  return {
    ...
    parameters: Type.Object({
      ...
      topic: Type.Optional(Type.String({ description: 'For help: overview, safety, or interaction.' })),
      url: Type.Optional(Type.String({ description: 'For start, new_tab, or navigate: absolute http(s) URL or about:blank.' })),
      tab_id: Type.Optional(Type.String({ description: 'For select_tab: tab ID returned by list_tabs or status.' })),
      ...
      max_content_length: Type.Optional(Type.Number({ description: 'For extract_content or evaluate: max output characters, up to 50000.' })),
