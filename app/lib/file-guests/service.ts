...
export function createFileGuestService(dependencies: {
  ...
  async function create(input: { workspace: WorkspaceContext; path: string; email: string; permission: FileGuestPermission; expiresAt: Date | null }) {
    ...
    const grant = await createCollaborationSessionGrant({ workspace: input.workspace, fileOptions: { workspace: input.workspace },
      request: { path: input.path, representation: 'auto', provider: 'yjs', ...COLLABORATION_CLIENT_CAPABILITIES } });
