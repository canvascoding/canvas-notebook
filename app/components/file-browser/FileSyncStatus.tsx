...
export function FileSyncStatus({ includeTreeStatus = false }: { includeTreeStatus?: boolean }) {
  ...
  const hasVisibleIssue = !isConnected || (includeTreeStatus && Boolean(error));

  if (!workspaceId || !hasVisibleIssue) return null;

  return <div role="status" aria-live="polite" className="flex h-8 shrink-0 items-center justify-between gap-2 border-b px-3 text-xs text-muted-foreground">
    <span className="min-w-0 truncate" title={error || undefined}>{!isConnected ? t('liveUpdatesUnavailable') : error}</span>
