...
function BreakdownTooltip({
  ...
  return (
    <div className="min-w-36 border border-border bg-popover px-3 py-2 text-xs text-popover-foreground shadow-md">
      ...
      <div className="font-medium tabular-nums">{formatInteger(payload[0]?.value)} Tokens</div>
    </div>
  );
}
...
export function UsageAnalyticsClient({ isAdmin }: UsageAnalyticsClientProps) {
  ...
  const formatUserOptionLabel = (userOption: UsageUserOption) =>
    t('filters.userOption', { label: userOption.label, count: formatInteger(userOption.usageEventCount) });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [dashboardRes, summaryRes, eventsRes, usersRes] = await Promise.all([
            ...
            {breakdownData.length ? (
              <ChartContainer config={breakdownChartConfig} className="min-h-[14rem] sm:min-h-[17rem]" aria-label={t('charts.breakdown.ariaLabel', { group: breakdownTitle })}>
                <BarChart data={breakdownData} layout="vertical" margin={{ top: 2, right: 14, left: 2, bottom: 2 }} barCategoryGap="24%">
                  ...
                  <Tooltip cursor={{ fill: 'var(--muted)' }} content={(props) => <BreakdownTooltip {...props} formatInteger={formatInteger} />} />
