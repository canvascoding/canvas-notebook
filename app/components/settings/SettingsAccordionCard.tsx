  ...
  return (
    <Collapsible open={isOpen} onOpenChange={onOpenChange}>
      <Card id={id} className={cn('gap-0 py-0', cardClassName)}>
          ...
          <CardHeader className="p-0">
            <CollapsibleTrigger asChild>
              <button
                ...
                aria-label={`${isOpen ? t('sections.collapse') : t('sections.expand')}: ${title}`}
              >
                <div className="flex w-full items-start justify-between gap-4">
                  ...
                  <div className="flex shrink-0 items-center gap-2 text-sm font-medium text-muted-foreground">
                    <span className="hidden sm:inline">
                      {isOpen ? t('sections.collapse') : t('sections.expand')}
                    </span>
        ...
        <CollapsibleContent>
          <CardContent className={cn(hideHeader ? 'space-y-3 p-0' : 'space-y-3 px-4 pb-4 pt-0 sm:px-6 sm:pb-6', contentClassName)}>
            {children}
          </CardContent>
        </CollapsibleContent>
      </Card>
