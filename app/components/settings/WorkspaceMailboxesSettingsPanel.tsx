...
type Mailbox = { id: string; workspaceId: string | null; workspaceName: string | null; emailAddress: string; displayName: string | null; smtpHost: string | null; smtpPort: number | null; smtpSecure: boolean | null; smtpUsername: string | null; imapHost: string | null; imapPort: number | null; imapSecure: boolean | null; imapUsername: string | null };
...
export function WorkspaceMailboxesSettingsPanel() {
  ...
  const mailboxPayload = () => ({ ...form, workspaceId: workspaceId || null, imapHost: form.imapEnabled ? form.imapHost : '', imapPort: form.imapEnabled ? form.imapPort : '', imapUsername: form.imapEnabled ? form.imapUsername : '', imapPassword: form.imapEnabled ? form.imapPassword : '' });
  const showCreate = () => { setEditing(null); setWorkspaceId(''); setForm(emptyForm); setError(null); setMessage(null); setOpen(true); };
  const showEdit = (mailbox: Mailbox) => { setEditing(mailbox); setWorkspaceId(mailbox.workspaceId || ''); setForm(formFromMailbox(mailbox)); setError(null); setMessage(null); setOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage(t(payload.data?.workspaceId ? 'savedAssigned' : 'savedUnassigned')); setOpen(false); setEditing(null); await load();
  ...
  return <><Card><CardHeader className="gap-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="space-y-1"><CardTitle className="flex items-center gap-2 text-base"><Inbox className="h-4 w-4" />{t('title')}</CardTitle><CardDescription>{t('description')}</CardDescription></div><Button type="button" size="sm" onClick={showCreate} disabled={loading || busy}><Plus className="mr-2 h-4 w-4" />{t('add')}</Button></div><p className="rounded-md border border-border bg-muted/30 px-3 py-2 text-sm leading-6 text-muted-foreground">{t('sharedHint')}</p></CardHeader><CardContent className="space-y-4">{error && <div className="flex gap-2 border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" />{error}</div>}{message && <div className="flex gap-2 border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-800 dark:text-emerald-300"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" /><div>{message}<Link href={`/${locale}/emails`} className="mt-1 block underline underline-offset-4">{t('openEmail')}</Link></div></div>}{loading ? <div className="flex items-center gap-2 py-6 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{t('loading')}</div> : mailboxes.length === 0 ? <div className="rounded-lg border border-dashed border-border px-4 py-8 text-center text-sm leading-6 text-muted-foreground">{t('empty')}</div> : <div className="space-y-3">{mailboxes.map((mailbox) => <article key={mailbox.id} className="rounded-xl border border-border bg-card p-4 shadow-xs sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate font-medium">{mailbox.emailAddress}</h3><Badge variant="outline">{mailbox.workspaceName || t('unassigned')}</Badge>{mailbox.imapHost ? <Badge variant="secondary">IMAP</Badge> : <Badge variant="secondary">{t('sendOnly')}</Badge>}</div>{mailbox.displayName && <p className="mt-1 text-sm text-muted-foreground">{mailbox.displayName}</p>}<p className="mt-3 text-xs text-muted-foreground">SMTP · {mailbox.smtpHost}:{mailbox.smtpPort}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => void testStored(mailbox)} disabled={busy}>{action === `test:${mailbox.id}` ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}{t('test')}</Button><Button type="button" size="sm" variant="outline" onClick={() => showEdit(mailbox)} disabled={busy}><Pencil className="mr-2 h-4 w-4" />{t('edit')}</Button><Button type="button" size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => void remove(mailbox)} disabled={busy}>{action === `remove:${mailbox.id}` ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}{t('remove')}</Button></div></div></article>)}</div>}</CardContent></Card><Dialog open={open} onOpenChange={(next) => !busy && setOpen(next)}><DialogContent className="flex max-h-[calc(100dvh-2rem)] max-w-4xl flex-col overflow-hidden"><DialogHeader><DialogTitle>{editing ? t('edit') : t('add')}</DialogTitle><DialogDescription>{t('editorDescription')}</DialogDescription></DialogHeader><div className="min-h-0 space-y-4 overflow-y-auto pr-1">{error && <p className="text-sm text-destructive">{error}</p>}<div className="space-y-2 rounded-lg border border-border bg-muted/30 p-3"><Label htmlFor="business-mailbox-workspace">{t('assignWorkspace')}</Label><select id="business-mailbox-workspace" value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} disabled={busy} aria-describedby="business-mailbox-sharing-hint" className="h-10 w-full min-w-0 rounded-md border border-input bg-background px-3 text-sm"><option value="">{t('assignLater')}</option>{editing?.workspaceId && !workspaces.some(workspace => workspace.id === editing.workspaceId) && <option value={editing.workspaceId}>{editing.workspaceName || editing.workspaceId}</option>}{workspaces.map(workspace => <option key={workspace.id} value={workspace.id}>{workspace.name}</option>)}</select><p id="business-mailbox-sharing-hint" className="text-sm leading-6 text-muted-foreground">{t(workspaceId ? 'assignmentSharingHint' : 'assignmentLaterHint')}</p>{workspaces.length === 0 && <p className="text-xs text-muted-foreground">{t('noAssignableWorkspaces')}</p>}</div>{message && <p role="status" className="text-sm text-emerald-700 dark:text-emerald-300">{message}</p>}<MailboxConnectionForm value={form} onChange={setForm} disabled={busy} isEditing={Boolean(editing)} /></div><div className="flex shrink-0 flex-col-reverse gap-2 border-t border-border pt-4 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={busy}>{t('cancel')}</Button><Button type="button" variant="outline" onClick={() => void testDraft()} disabled={busy}>{action === 'test-draft' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{t('test')}</Button><Button type="button" onClick={() => void save()} disabled={busy}>{action === 'save' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}{t('testAndSave')}</Button></div></DialogContent></Dialog></>;
}
