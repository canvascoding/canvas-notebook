      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-5">
          ...
          <Card>
            ...
            <CardContent className="space-y-3"><Textarea value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={800} placeholder="e.g. Prefers short, decisive weekly updates." disabled={scope === 'agent' && selectedAgentOwner?.status === 'deleted'} /><div className="flex items-center justify-between gap-3"><span className="text-xs text-muted-foreground">{scope === 'agent' && selectedAgentOwner?.status === 'deleted' ? 'Deleted-agent memory is read-only until transferred.' : `${draft.length}/800`}</span><Button onClick={() => void addEntry()} disabled={!draft.trim() || adding || !canUseScope || (scope === 'agent' && selectedAgentOwner?.status === 'deleted')}>{adding ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Plus className="mr-2 size-4" />}{scope === 'workspace' || scope === 'organization' ? 'Suggest memory' : 'Save memory'}</Button></div></CardContent>
          </Card>
