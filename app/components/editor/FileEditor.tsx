...
export function FileEditor({ onClosePreview }: FileEditorProps = {}) {
  ...
  const documentIdentity = `${currentFileWorkspaceId}:${currentFile?.editorIdentity ?? currentFile?.viewId ?? currentFile?.collaboration?.document?.id ?? currentFilePath}`;
  ...
  const textSession = useTextCollaborationSession({ enabled: Boolean(currentFile?.collaboration?.crdtCapable),
    ...
    expectedDocumentId: currentFile?.collaboration?.document?.id });
  const activeCollaborationDocument = useCollaborationDocument({
    enabled: Boolean(currentFile?.collaboration?.crdtCapable),
    documentKey: currentFile?.editorIdentity,
    waitForSession: true,
    workspaceId: currentFileWorkspaceId, path: currentFilePath ?? undefined,
    representation: textSession.session && textSession.session.representation !== 'excalidraw_scene'
      ? textSession.session.representation : 'plain_text',
  ...
  }, [activeExternalTextChangePath, activePath, currentFile?.path, currentFile?.unavailable, currentFile?.collaboration?.crdtCapable, currentFile?.collaboration?.sceneCapable, draft, handleSaveError, isDirty, markSaved, markSaving, saveTrackedFile, setSaveError]);
