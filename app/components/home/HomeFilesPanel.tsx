...
import { HomeFileRowsSkeleton, HomeSkeleton } from './HomeSkeletons';
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) { setFailed(true); setResult(null); }
      } finally {
        if (!controller.signal.aborted) setLoading(false);
      }
    }, query ? 220 : 0);
    return () => { controller.abort(); window.clearTimeout(timer); };
  }, [workspaceId, view, query, expanded, revision]);
  ...
  return (
    <section aria-labelledby="home-files-heading" className="min-w-0" data-testid="home-files">
      ...
      <div className="relative mb-4">
        ...
        <Input disabled={!workspace} aria-label={t('search')} placeholder={t('search')} value={query} onChange={(event) => { setQuery(event.target.value); setLoading(true); }} className="h-10 pl-9 pr-10" maxLength={256} />
