...
export function closeMemoryReview() {
  loadGeneration += 1;
  useMemoryReviewStore.setState({ open: false, activeEntry: null, loading: false, deciding: null, error: null });
}

export async function retryActiveMemory() {
  const state = useMemoryReviewStore.getState();
  if (!state.queue[state.activeIndex] || state.loading) return;
  await loadAt(state.activeIndex, state.queue);
}
