...
export function ModelLibrary() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl min-w-0 flex-col gap-4 sm:gap-5">
      <div className="flex min-w-0 flex-col gap-4 border-b border-border/80 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0 space-y-3">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Library</p>
            <h1 className="mt-1 truncate text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">{t('tabs.models')}</h1>
          </div>
          <div className="grid w-full min-w-0 grid-cols-3 border border-border bg-card p-1 shadow-sm sm:inline-grid sm:w-auto">
            {tabs.map(({ key, label, count, Icon }) => (
              <button
                key={key}
                onClick={() => setActiveTab(key)}
                className={cn(
                  'flex min-h-10 min-w-0 items-center justify-center gap-1 px-2 text-xs font-medium transition-colors hover:bg-accent/60 hover:text-foreground sm:gap-2 sm:px-4 sm:text-sm',
              ...
              >
                ...
                <span className={cn(
                  'shrink-0 border px-1 py-0.5 text-[10px] leading-none sm:ml-1 sm:px-1.5 sm:text-[11px]',
        ...
        <Button onClick={handleCreate} className="h-11 w-full justify-center gap-2 px-5 shadow-sm sm:w-auto lg:self-start">
          <Plus className="h-4 w-4" />
          {createLabel}
        </Button>
      </div>

      <div className="relative min-w-0 border border-border bg-card shadow-sm">
      ...
      {activeTab === 'products' && (
        productsLoading ? (
          ...
          <div className="flex min-h-[240px] flex-col items-center justify-center gap-2 border border-dashed border-border bg-card/60 p-4 text-center sm:min-h-[260px] sm:p-8">
          ...
          <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
