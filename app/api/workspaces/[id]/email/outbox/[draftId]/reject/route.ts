...
import { rejectWorkspaceOutboxDraft } from '@/app/lib/email/workspace-inbox-outbox';

export async function POST(request: NextRequest, context: { params: Promise<{ id: string; draftId: string }> }) {
  ...
  try {
    ...
    const data = await rejectWorkspaceOutboxDraft({ userId: session.user.id, workspaceId: id, draftId, expectedVersion });
