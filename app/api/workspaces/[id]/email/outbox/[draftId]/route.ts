...
import { findWorkspaceOutboxDraft, updateWorkspaceOutboxDraft } from '@/app/lib/email/workspace-inbox-outbox';
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string; draftId: string }> }) {
  const session = await auth.api.getSession({ headers: request.headers });
  if (!session) return NextResponse.json({ success: false, error: 'Unauthorized' }, { status: 401 });
  try {
    const { id, draftId } = await context.params;
    const data = await findWorkspaceOutboxDraft(session.user.id, id, draftId);
