...
function errorResponse(error: unknown): NextResponse {
  ...
  console.error('[API] Mobile workspace members failed:', error);
  return NextResponse.json(
    { success: false, code: 'INTERNAL_ERROR', error: 'Workspace members could not be loaded.' },
...
export async function GET(
  ...
  const limited = rateLimit(request, { limit: 60, windowMs: 60_000, keyPrefix: 'mobile-workspace-members' });
  ...
  try {
    ...
    if (!workspaceId.trim() || selectedWorkspaceId !== workspaceId.trim()) {
      return NextResponse.json(
        { success: false, code: 'WORKSPACE_CONTEXT_MISMATCH', error: 'Select this workspace to view its members.' },
        { status: 409, headers: responseHeaders },
      );
    }
    const data = await listMobileWorkspaceMembers({
