...
import { POST as disablePlugin } from '@/app/api/plugins/[name]/disable/route';
...
export async function POST(
  ...
  const response = await disablePlugin(request, context);
  ...
  if (!response.ok || !payload?.success || !payload.plugin) {
    return NextResponse.json({
      success: false,
      error: payload?.error || 'Failed to disable plugin',
