...
import { revertAgentOperation } from '@/app/lib/collaboration/agent-operations';
...
export async function POST(request: NextRequest, context: { params: Promise<{ operationId: string }> }) {
  ...
  const limited = applyRateLimit(request, { limit: 30, windowMs: 60_000, keyPrefix: 'collaboration-operation-revert' });
  ...
  try {
    ...
    const operation = await revertAgentOperation({
      operationId,
    ...
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : 'Revert failed.' }, { status: 409 });
  }
}
