...
async function main() {
  ...
  const providerMessages = await normalizePiMessagesForLlm(metadataLoaded!.messages);
  assert.deepEqual(providerMessages.slice(0, 2), metadataMessages.slice(0, 2), 'assistant replay signatures survive provider normalization');
  const { details: _displayDetails, sequence: _sequence, ...providerTool } = metadataMessages[2] as typeof piToolMetadataFixture & { sequence?: number };
  assert.deepEqual(providerMessages[2], providerTool, 'tool protocol content survives; display details and database sequence stay out of the provider payload');
