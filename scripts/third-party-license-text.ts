  ...
  for (const text of texts) {
    ...
    for (const line of text.split('\n')) {
      ...
      const isLicenseBoilerplate = (
        ...
        || /\b(?:keep|keeping|preserve|preserving)\b.*\bcopyright (?:notices?|comments?|information)\b/iu.test(normalized)
        || /\b(?:contain|contains|containing)\b.*(?:@copyright|\bcopyright\b)/iu.test(normalized)
        || /\b(?:place|placing)\s+copyright (?:information|notices?)\b/iu.test(normalized)
