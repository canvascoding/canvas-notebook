...
assert.match(composeControllerSource, /void openEmailReview\(/u);
assert.doesNotMatch(composeControllerSource, /persistOutboxDraft|outboxEditing/u);
...
assert.match(reviewCenterSource, /loadEmailReviewQueue/u);
assert.match(reviewCenterSource, /email_review_updated/u);
assert.match(reviewCenterSource, /email-app-review-open/u);
assert.match(reviewCenterSource, /filter: 'failed'/u);
assert.doesNotMatch(reviewCenterSource, /<Dialog/u);
