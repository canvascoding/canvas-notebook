...
- Scratch directories use private permissions, symlink-free managed ancestry, active-operation leases, inactive cleanup, and configurable byte/file limits (`CANVAS_AGENT_RUNTIME_TEMP_MAX_BYTES`, `CANVAS_AGENT_RUNTIME_TEMP_MAX_FILES`). Quota accounting does not follow symlinks, counts temporary staging headroom needed for crash-safe binary replacement, and clears disposable session scratch contents before returning a Bash overage error.
