...
- Schritt 5: Abgeschlossene Assistant-/Tool-Blöcke teilen sich höchstens 6.000 geschätzte Tokens bzw. 15 % des effektiven Kontextfensters, einzelne Ergebnisse höchstens 5 %. Gemessen werden Text und serialisierte Protokollfelder samt Escaping; Bilder bleiben im bestehenden Multimodal-Budget. Kleine neue Ergebnisse erhalten ebenfalls Policy-Metadaten, damit mehrere mittlere Antworten gemeinsam ausgelagert werden können. Basisinhalt bleibt für Modellwechsel erhalten; persistierte Ansichten tragen Policy, Modell-/Blockkennung und Quell-Fingerabdruck. Quellenbereiche ermöglichen kleinere Web-Ansichten ohne Verlust späterer Quellen. Leseansichten berechnen `nextOffset` aus dem tatsächlich sichtbaren UTF-16-Fenster neu.
- Gemeinsame Laufzeitgrenze: History-Projektion, kanonisches Payload und Live-/gespeicherte Kontextmessung verwenden dieselbe Blockberechnung. Live-Chat, Automationen und kurzlebige delegierte Worker sichern neu benötigte Originale und persistieren Modellansichten. Interne `details` und Tool-Usage gehen nicht ins Provider-Payload; `addedToolNames` bleibt erhalten. Delegierte Worker prüfen jetzt das vollständige Payload mit aktuellem Systemprompt und demselben Antwortlimit wie der Provider-Aufruf. Worker und Automationen warten die direkte Loop-Funktion ab, damit Ablehnungen der Vorbereitung abgefangen werden und Teilergebnisse erhalten bleiben.
- Budget-Prüfungen: `npm run test:pi:tool-output-budgets` prüft 16k/32k/262k mit derselben gemischten Folge, unabhängigen Modellanbietern, Originalablage, Persistenzprojektion/Neuladen, Modellwechsel, Wiederholungen, Unicode-Offsets und echter Überlastung. Der echte `LivePiRuntime` und die Kontextmessung stimmen mit dem kanonischen Payload überein. Der delegierte Worker läuft im Test mit der echten direkt abwartbaren Agent-Schleife und simuliertem Provider/Persistenzziel. Bestehende normalisierte Preflight-, Kontextbudget-, Multimodal- und Automation-Runtime-Kompaktionstests sind grün; die vorhandenen Message-Projection- und Automation-History-Suites benötigen PostgreSQL für ihren DB-Teil.
...
- Schritt 6: Tool-Karten zeigen Quellenzahlen, verfügbare Originale und Speicherfehler. Der neue GET-Endpunkt `/api/sessions/[sessionId]/tool-output` prüft Anmeldung, Sitzungsbesitz, Agentzugriff und Workspace vor jedem Lesen. Originale werden ausschließlich nach Klick und als begrenztes UTF-16-Fenster geladen; Zurück/Weiter und direkte Zeichenposition sind verfügbar. Sitzungswechsel verwerfen die geladene Ansicht und brechen offene Anfragen ab. Inhalte werden als Klartext dargestellt und erzeugen keine Modellnachrichten.
- Abschließender Nachlese-Check: Gespeicherte JSON-Dateien mit mehreren Megabytes in einer einzigen Zeile werden mit `rg` über reine Match-Offsets durchsucht. Höchstens 20 kurze Trefferauszüge mit UTF-16-Lesepositionen verhindern übergroße Suchausgaben, auch bei gierigen Mustern. Bestehende Workspace-Suchen behalten ihren bisherigen Vertrag.
- Erweiterte Bereinigung: Nach erfolgreicher Prüfung der gespeicherten Referenzen werden alte unveröffentlichte Dateien innerhalb inaktiver Sitzungen unter derselben Kernel-Sperre wie Schreibzugriffe entfernt. Referenzierte Dateien, frische Ausgaben und frische Clone-Zwischenstände bleiben erhalten. Datenbank-/History-Fehler stoppen die Bereinigung. Alte private Zwischenstände abgebrochener Forks werden ebenfalls entfernt.
- Delegationsgrenze: Worker speichern und lesen unter ihrer eigenen Sitzungsidentität. Elternreferenzen werden nicht automatisch freigeschaltet; gezielte Textübergabe nutzt den vorhandenen Delegationskontext. Es wurde keine pauschale sitzungsübergreifende Leseberechtigung eingeführt.
- Abschließende automatische Tests: `test:pi:tool-outputs`, `test:pi:web-outputs`, `test:pi:external-outputs`, `test:pi:tool-output-budgets` und `test:pi:tool-output-preview` bestehen. Die Vorschau-Suite prüft echte temporäre Dateien am Endpunkt mit simulierten Zugriffsrechten sowie React/JSDOM-Komponenten in Deutsch und Englisch. Bestehende Chat-Fork-, Kontextbudget-, normalisierte Preflight- und Multimodal-Regressionen bestehen ebenfalls. Typecheck, ESLint der geänderten Dateien und `git diff --check` sind grün.
- Produktionsbuild: `npm run build` erfolgreich, einschließlich Lizenzprüfung und Next.js-Typecheck. Dynamische DATA-Pfade beim selektiven Fork sind als reine Laufzeitpfade markiert; die zunächst gefundene breite Turbopack-Dateisuche ist im finalen Build behoben. Die lokale Build-Umgebung meldet weiterhin die fehlende MCP-OAuth-Basis-URL; es wurden dafür keine Zugangsdaten oder lokalen Einstellungen verändert.
- Prüfgrenzen: Keine Live-Provider-Abfragen, kein vollständiger PostgreSQL-Backup/Restore-Lauf und keine DB-abhängigen Bestandssuites ohne konfigurierte Testdatenbank. Speicher-Lifecycle und Adapter werden mit echten temporären Dateien und gezielten DB-/Provider-Mocks getestet. Der Datenverzeichnis-Umzug ist separat abgedeckt.
