...
| Third-Party License Compliance | einzelne Package-/Seed-Skill-Lizenzmetadaten, keine zentrale Notice-Liste | `instance/release` Inventar pro Artefakt mit verifizierten Lizenztexten, Copyright-Hinweisen und Policy-Entscheidung | Alle npm-/Asset-/Native-/Container-Komponenten erfassen, Notices generieren und Drift im CI blockieren | P0/P9 |
...
| Excalidraw Collaboration | eingebetteter npm-Editor mit komplettem JSON-Autosave und `isCollaborating=false` | `workspace` Datei plus eigener Excalidraw Scene Provider, elementweise Reconciliation, Asset Store, Postgres-State, Checkpoints und Presence | Architektur-Spike; gemeinsame Auth-/Transport-/Presence-Infrastruktur aus P7b nutzen, aber separate Aufgabe `52` und kein Y.Text-JSON | P7/P7c/P9 |
