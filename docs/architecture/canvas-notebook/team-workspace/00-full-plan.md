...
- Agent-Koautorenschaft braucht zusaetzlich eine persistierte idempotente Operationszustandsmaschine, serialisierte parallele Runs, Cancel-/Restart-Schutz, isolierten Schema-/Stable-ID-Preflight, aktuelle Review-Revalidierung und getrennte Status fuer live angewendet, in Yjs persistiert und als Datei materialisiert. IME/Unicode-Grenzen, Encoding, GC/Compaction, Feedback-Loops, Ressourcenlimits und Datei-Lifecycle-Generationen sind verbindliche Hardening-Gates von Aufgabe `48`.
