...
- Production build and changed-file lint pass. The authorized HTTPS browser check confirms file creation/read without an explicit workspace header, owner-only read-only legacy recovery, HTML preview with external image/local module/worker, mobile redirect and ticket revocation. Its repeated PDF request correctly received the existing high-load 503 while the host was heavily loaded; the preceding package 3 PDF render remains the successful UI image/layout reference. Final-build verification is recorded below. Before the native check, the local legacy fixture was inventoried (9 files, 64 bytes); migration retains those source files.

## Package 5: targeted dependency updates

A fresh npm audit of the implementation baseline reported 13 affected packages (nine high, four moderate). After the targeted updates it reports two high-severity affected packages, both in the existing `pptxgenjs -> image-size` chain; no moderate or critical findings remain in that audit. These are package counts, not 13 independent vulnerabilities. The lockfile was also compared with newly fetched `origin/main` at `aed0c6be`: main had not changed that lockfile since the implementation baseline. The additional main commits have not been merged into this branch.

| Chain | Change and result |
| --- | --- |
| Marp CLI / Puppeteer / archive extraction | Marp CLI 4.5.0 → 4.5.1, with a scoped `puppeteer-core` 25.3.0 override matching the app's existing browser client. This removes the vulnerable `extract-zip` chain. Root app Puppeteer remains 25.3.0. |
| mailparser / HTML-to-text | mailparser 3.9.14 → 3.9.23 brings html-to-text 10.0.1 and deepmerge-ts 8.0.2, removing the cyclic-graph stack-overflow advisory. |
| MathJax / speech-rule-engine / XML | Keep speech-rule-engine 4.1.4 and scope its XML parser to `@xmldom/xmldom` 0.9.12, which fixes entity-reference injection. Marp core and its existing MathJax generation are retained. |
| PptxGenJS / image-size | Retain the existing versions. The current image-size advisories cover releases through 2.0.2 without a fixed release. npm's proposed PptxGenJS 1.1.5 downgrade is not a compatible security update. |

The residual image-size issues can hang image parsing for crafted ICNS/JXL/HEIF content. No direct PptxGenJS/image-size import was found in `app/` or `server/`; this dependency is available to agent-generated scripts through the installed runtime. This review found no anonymous HTTP path into that parser, but agent/script exposure remains. Follow-up is a tested upstream fix or separately scoped parser replacement; the audit is not described as clean. Terminal and agent Bash remain deferred.

Advisory evidence: [extract-zip](https://github.com/advisories/GHSA-jmr9-qjv8-65gv), [deepmerge-ts](https://github.com/advisories/GHSA-ggr8-5vv4-36mx), [xmldom](https://github.com/advisories/GHSA-6gmq-8vp8-gcm6), [image-size ICNS](https://github.com/advisories/GHSA-w3rx-r6r6-pgpr), [image-size JXL/HEIF](https://github.com/advisories/GHSA-5p2g-fcmc-qvqq). The Puppeteer major override was checked against its [release changes](https://github.com/puppeteer/puppeteer/releases/tag/puppeteer-core-v25.0.0) and exercised with the real Marp CLI; it was tested on Node 26.7, not separately on the minimum supported Node 22 runtime.

Verification:

- `npm run test:security:dependencies` passes: encoded MIME headers, HTML conversion, PNG attachment, XML injection rejection, PptxGenJS text/image presentation, and real Marp PDF, two PNG pages and a two-slide PowerPoint export. Fixtures include MathJax, a relative SVG and a public HTTPS image. Both rendered slide images were visually inspected.
- The PDF credential/context and independent network-boundary regressions also pass after the final lockfile reconciliation: no external app credentials, isolated browser jobs, zero private recording-server requests, blocked worker/WebSocket probes and preserved public images. `npm ls` validates the installed versions, and `npm ci --dry-run --ignore-scripts` accepts the lockfile.
- Existing Marp preview, Markdown LaTeX/rich-block PDF and agent dependency-bridge tests pass. The older email MIME/header integration test cannot reach its assertions because the unchanged Pi SDK does not export `@earendil-works/pi-ai/compat`; it is not reported as passing. The new MIME regression exercises the changed parser directly.
- The complete production build, TypeScript and changed-file ESLint pass. Generated third-party notices, inventory and cache were refreshed; the license compliance gate reports no release blockers. The new mailsplit version retains the same byte-identical MIT license already elected for the previous version, with immutable source evidence in the [technical review record](../compliance/third-party-review-decisions.md). No global release approval was changed.
- The final built app was restarted and checked through both HTTPS hostnames. Default workspace file operations, owner-only recovery, external/local images, root-relative modules, workers, mobile redirection, blocked app access and ticket revocation pass. After a bounded four-minute wait for host load to fall, the UI PDF endpoint still returned the unchanged `BROWSER_EXPORT_HIGH_LOAD` 503. The final UI run is therefore **not** recorded as a successful PDF download or pixel comparison. Successful evidence consists of the package 3 full UI download/layout comparison and the final dependency/PDF/network renderer suites above. Repeat the full download on an idle host as part of rollout verification; no application resource guard was relaxed. The seven new fixture files were removed and the test session signed out.

All five packages are local implementation work. Production DNS/TLS, ingress synchronization, deployment and a public-host verification remain rollout requirements; no container, production host or DNS record was changed.
