...
PostgreSQL bigint timestamps are Unix epoch milliseconds. Startup normalizes
plausible legacy epoch-second values once, under a PostgreSQL advisory
transaction lock. The range predicate makes the operation safe to repeat and
prevents already-normalized millisecond values from being scaled again.
