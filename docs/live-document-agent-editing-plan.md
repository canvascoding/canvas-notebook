...
Der anschließende [Abgleich mit allen 13 Abnahmeszenarien](live-document-browser-acceptance.md) benennt zusätzliche Lücken in den vorhandenen Browserfällen: getrennte Clients verwenden bisher denselben Account, der Neustarttest wartet bereits auf Markdown, und der PDF-Test wechselt noch nicht innerhalb derselben Seite zurück zum Editor. Offline-/Prozessabbrüche, Gast-Rechte und echte IME-Eingaben benötigen ebenfalls eigene integrierte Nachweise. Die neun vorbereiteten Dateien allein schließen die Abnahmematrix deshalb nicht ab.
