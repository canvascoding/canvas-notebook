...
### Ergänzung: sichtbare Cursorbeschriftungen in Tabellen und am Rand

Die neue Browserregression reproduzierte eine am oberen Editor-Rand um rund 14 Pixel abgeschnittene Beschriftung; eine zweite Gegenprobe scheiterte an der eigenen Scrollgrenze einer Tabelle. Die Position berücksichtigt jetzt den sichtbaren Editor, den Visual Viewport und alle auf der jeweiligen Achse abschneidenden Vorfahren. Bei fehlendem Platz oberhalb erscheint die Beschriftung unterhalb des Cursors. Ein eigener Plugin-View bündelt Scroll-, Resize- und Layoutänderungen pro Frame und entfernt Listener, ResizeObserver und ausstehende Frames bei jedem Abbau. Dokumentinhalt und Awareness-Protokoll bleiben unverändert.

Auf Image `7b53ab0135a1` (Start 12.09.2026, 00:36:22 UTC) bestehen die tatsächlichen Zwei-Nutzer-Fälle in Chromium (`step7-table-caret-fixed-chromium-02`, 8,9 Sekunden) und WebKit mit Touch-Viewport (`step7-table-caret-fixed-webkit-02`, 9,3 Sekunden): Tabellenkopf, Verkleinern auf 390 × 544, Scrollen ohne erneutes Hover, wiederholtes Lesen/Bearbeiten und weitere native Peer-Eingabe. Screenshots sind visuell geprüft. Ein erster Lauf bestand die Randprüfungen, verwendete aber eine plattformabhängige End-Tasten-Annahme für die anschließende Peer-Eingabe. Der Test prüft jetzt deren tatsächliche native Konvergenz. WebKit meldet den nicht unterstützten optionalen `interactive-widget`-Viewport-Hinweis; ausschließlich diese genaue Browsermeldung bleibt als Warnung erhalten, alle anderen Konsolen- und Seitenfehler bleiben Fehler.

Der zusätzliche tatsächliche Plugin-View-Test bestätigt dreimaligen View-Austausch, Ereignisbündelung und vollständigen Abbau auch mit bereits eingeplantem Callback. Er ist Teil von `test:editor:presentation`. Die Presentation-Suite, Gastwerkzeuge, strukturelle Wiederherstellung, TypeScript, ESLint sowie beide Produktionsbuilds bestehen. Die JSDOM-Fixtures stellen ihren vorhandenen ResizeObserver nun auch am zugehörigen Window bereit. Die unabhängige Codeprüfung fand keine weiteren blockierenden Befunde.

Die drei übrigen tatsächlichen Live-Browserfälle bestehen ebenfalls auf demselben Image (`step7-final-live-chromium`, 1,4 Minuten): ruhiges Löschen/Verschieben, Cursor und Auswahl mit Wiederverbindung sowie Read/Edit/Patch mit Agentenfreigabe und Wiederöffnung. Die erneute Syntaxprüfung besteht in den ersten beiden Fällen; der dritte findet eine separate Startlücke der Live-Quellansicht. Diese wird vor Abschluss der integrierten Abnahme weiter untersucht.
