...
function scheduleExpiredSessionCleanup() {
  try {
    ...
    // PostgreSQL startup migrations normalize persisted timestamps to epoch milliseconds.
    const cleanupQuery = isPostgres
      ? 'DELETE FROM session WHERE expires_at < floor(extract(epoch from now()) * 1000)::bigint'
