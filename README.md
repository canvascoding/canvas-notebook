...
| `BOOTSTRAP_ADMIN_EMAIL` | No | Optional recovery/bootstrap login email — created or updated when paired with `BOOTSTRAP_ADMIN_PASSWORD` |
| `BOOTSTRAP_ADMIN_PASSWORD` | No | Optional recovery/bootstrap password; prefer the setup UI or CLI recovery command for normal installs |
| `BOOTSTRAP_ADMIN_NAME` | No | Optional recovery/bootstrap display name (default: `Administrator`) |
