...
The desktop app is only a client. To run the Canvas Notebook server locally on macOS or Windows, run the platform installer. It downloads the portable `canvas-notebook` CLI from the latest GitHub release, installs a user-local command wrapper, pulls the pre-built Docker image, starts the server on `http://localhost:3456`, and registers a user-level startup service.
...
The installers can also be run from a repository checkout or from the packaged CLI bundle. By default they use the latest CLI release asset; pin a version for reproducible installs:
...
```powershell
$env:CANVAS_VERSION = "v2026.7.3.2"; irm https://raw.githubusercontent.com/canvascoding/canvas-notebook/main/install/windows.ps1 | iex
```
...
Set `CANVAS_INSTALL_SERVICE=false` before running the installer if you want to skip startup service registration. Set `CANVAS_OPEN_BROWSER=false` to skip opening the browser after install.
...
| `BETTER_AUTH_TRUSTED_ORIGINS` | No | Additional allowed browser origins, comma-separated; applies to Better Auth and the chat WebSocket |
