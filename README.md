...
The installer will:
1. Ask whether to use the recommended pre-built image or install from source
2. Install **Docker** if it is missing
3. Auto-generate secrets and write the local config
4. Ask for the public URL/domain
5. Pull the latest pre-built image from `ghcr.io` and start the container
6. Install the host-side `canvas-notebook` management command and systemd service
7. Optionally install and configure **Caddy** for automatic HTTPS, only if you choose it
