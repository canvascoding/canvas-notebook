...
`ADMIN_EMAIL`, `ADMIN_PASSWORD`, and optional `ADMIN_NAME` are used once to create or reset the initial admin through the host CLI. They are not written to `config.json`, `.env`, or Compose files. If you omit them, open the app and create the first admin in the setup UI.
...
Create the first admin in the setup UI, pass `ADMIN_EMAIL`/`ADMIN_PASSWORD` once to the Linux installer, or use `canvas-notebook admin reset-password --email ... --password-stdin` on the VM host.
