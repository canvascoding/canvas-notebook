...
description: "ALWAYS use this skill when working with Remotion, React video creation, programmatic video generation, video compositions, or animation in React. Covers project setup, compositions, sequences, rendering, audio, video embedding, transitions, timing, captions, and advanced patterns. Triggers: remotion, video creation, react video, programmatic video, animation, composition, render video, create video."
compatibility: "Requires a Remotion project with @remotion/cli and remotion installed"
metadata:
  version: "1.0"
  author: remotion-dev
---
...
## Canvas Notebook Notes

- Remotion is a pure instruction skill — no runtime dependencies are needed in the container beyond what's already installed (ffmpeg, chromium, Node.js)
- To render videos with Remotion in the workspace, run `npx remotion render` in the project directory
- The container already has ffmpeg and chromium installed for Remotion rendering
- Assets for Remotion projects should be placed in the project's `public/` directory within `/data/workspace/`